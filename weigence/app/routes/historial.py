...
def historial():
    ...
    if session.get('last_page') != 'historial':
        ...
        registrar_evento_humano('navegacion', f"{usuario_nombre} ingreso al Historial")
            ...
            movimientos.append(
                {
                    ...
                    'tipo_evento': m.get('tipo_evento', 'N/A'),
                    'cantidad': m.get('cantidad', 0),
                    'ubicacion': f"Estante {estante}" if estante else 'N/A',
        ...
        errores = [
            {
                ...
                'descripcion': e.get('descripcion', 'Sin descripcion'),
