...
def dashboard():
    ...
    if session.get('last_page') != 'dashboard':
        ...
        registrar_evento_humano('navegacion', f"{usuario_nombre} ingreso al Dashboard")
...
def api_dashboard_filtrado():
    ...
    if rango == 'hoy':
        ...
        end = datetime(year + 1, 1, 1) if mes == 12 else datetime(year, mes + 1, 1)
    ...
    productos_top = [
        {
            ...
            'ventas': round(float(p.get('ventas', 0)), 0),
        }
