...
class IAFormatter:
    ...
    def render(self, insight: EngineInsight, snapshot: IASnapshot) -> Dict[str, str]:
        ...
        if not template:
            logger.warning(
                "[IAFormatter] Plantilla '%s' no encontrada, se utilizará fallback",
                insight.template,
            )
    ...
    def _construir_contexto(
        self, insight: EngineInsight, snapshot: IASnapshot
    ) -> Dict[str, float]:
        contexto = {
    ...
    def _construir_mensaje(
        ...
        for modulo in seleccion:
            try:
                ...
                logger.exception("[IAFormatter] Error formateando módulo '%s': %s", modulo, exc)
