    ...
    try {
      ...
      data.forEach(e => {
        ...
        if (e.id_estante >= 6) {
          ...
          const pesaIconClass = pesaActiva ? 'text-green-500' : 'text-red-500';
          const pesaTextClass = pesaActiva ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
          const pesaIcon = pesaActiva ? 'check_circle' : 'error';
          const pesaTexto = pesaActiva ? 'Sensor Activo' : 'Sensor Inactivo';
          pesaIndicador = `
            <div class="flex items-center gap-1 mt-2 text-xs">
              <span class="material-symbols-outlined ${pesaIconClass}" style="font-size: 16px;">${pesaIcon}</span>
              <span class="${pesaTextClass} font-medium">${pesaTexto}</span>
            </div>
          `;
        }
